defmodule Rumbl.AuthTest do
  ...
  test "login puts the user in the session", %{conn: conn} do
    # Make a request using the original connection, logging in the user along
    # the way.
    login_conn =
      conn
      |> Auth.login(%Rumbl.User{id: 123})
      |> send_resp(:ok, "") # Directly send a successful response to client

    # Make a new request
    next_conn = get(login_conn, "/")
    # Assert that the session for the new request contains the user id
    assert get_session(next_conn, :user_id) == 123
  end

  test "logout drops the session", %{conn: conn} do
    # Make a request that logs the user out
    logout_conn =
      conn
    ...
    # Ensure the session does not contain the user id
